...
process_pair_ready_read(data_t *data)
{
    // change text
    char recv_text[2048];
    int count = read(data->fd, &recv_text, sizeof(recv_text));
...
int main(int argc, char **argv)
{
    ...
    int j = 0;  // number of current fd in epoll
    while (1) {
        if(MUST_EXIT == 1){
            break;  // signal
        }
        ...
        if (current_connect != -1) {    // if there is client and no signal
            ...
            make_non_blocking(current_connect);
            create_event_data(epoll_fd, current_connect); // add nonblock, metadata, and push in epoll

            entries[j]->fd = current_connect;
            j++;
...
//            }
        ...
        // processing events
        int ev_count = epoll_wait(epoll_fd, ready_fds, 256, 100);
        if (ev_count < 0) {
            perror("epoll_wait");
    ...
    for (int i = 0; i < j; ++i) {
        shutdown(entries[i]->fd, SHUT_RDWR);
        close(entries[i]->fd);
    }
