    ...
    while (1) {
        ...
        if (current_connect != -1) {    // if there is client and no signal
            ...
            if (-1 == faccessat(dir_fd, file_name, F_OK, 0)) {       //file do not exist
                write(current_connect, "HTTP/1.1 404 Not Found\r\n", strlen("HTTP/1.1 404 Not Found\r\n"));
                write(current_connect, "Content-Length: 0\r\n", strlen("Content-Length: 0\r\n"));
                write(current_connect, "\r\n", strlen("\r\n"));
            ...
            if (-1 == faccessat(dir_fd, file_name, R_OK, 0)) {
                write(current_connect, "HTTP/1.1 403 Forbidden\r\n", strlen("HTTP/1.1 403 Forbidden\r\n"));
                write(current_connect, "Content-Length: 0\r\n", strlen("Content-Length: 0\r\n"));
                write(current_connect, "\r\n", strlen("\r\n"));
            ...
            if (-1 == faccessat(dir_fd, file_name, X_OK, 0)) {
                ...
                pid_t pid = fork();
                if(0 == pid){
                    if( -1 == dup2(current_connect, 1)) {   // send output to client
                        close(current_connect);
                        exit(1);
                    }
                    close(current_connect);
                    ...
                    strcat(command, file_name);  // command to exec
                    if (-1 == execl(command, command, NULL)){
                        close(current_connect);
                        exit(1);
                    }
                } else{
